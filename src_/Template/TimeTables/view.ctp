...
<div class="timeTables view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        <tr>
            ...
            <td><?= $timeTable->has('master_role') ? $this->Html->link($timeTable->master_role->id, ['controller' => 'MasterRoles', 'action' => 'view', $timeTable->master_role->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Master Class') ?></th>
            <td><?= $timeTable->has('master_class') ? $this->Html->link($timeTable->master_class->id, ['controller' => 'MasterClasses', 'action' => 'view', $timeTable->master_class->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Master Section') ?></th>
            <td><?= $timeTable->has('master_section') ? $this->Html->link($timeTable->master_section->id, ['controller' => 'MasterSections', 'action' => 'view', $timeTable->master_section->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Master Subject') ?></th>
            <td><?= $timeTable->has('master_subject') ? $this->Html->link($timeTable->master_subject->id, ['controller' => 'MasterSubjects', 'action' => 'view', $timeTable->master_subject->id]) : '' ?></td>
        </tr>
        <tr>
